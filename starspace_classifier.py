...
class EmbeddingIntentClassifier:
    ...
    def _load_flag_if_tokenize_intents(self):
        ...
        if self.intent_tokenization_flag and not self.intent_split_symbol:
            print("intent_split_symbol was not specified, "
    ...
    def _check_hidden_layer_sizes(num_layers, layer_size, name=''):
        ...
        if isinstance(layer_size, list) and len(layer_size) != num_layers:
            ...
            print("The length of hidden_layer_size_{} = {} "
    ...
    def _output_training_stat(self,
        ...
        print("epoch {} / {}: loss {}, train accuracy : {:.3f}"
              "".format((ep + 1), self.epochs, sess_out.get('loss'), train_acc))
    ...
    def prepare_training_data(self, X, y):
        ...
        self.vect = CountVectorizer(ngram_range=(1, 1),
        ...
        self.vect = self.vect.fit(X)

        X = self.vect.transform(X).toarray()
    ...
    def train(self, X, y):
        ...
        if len(intent_dict) < 2:
            print("Can not train an intent classifier. "
        ...
        print("Check if num_neg {} is smaller than "
              "number of intents {}, "
    ...
    def transform(self, query):
        ...
        vectorized = self.vect.transform([query])
        vectorized = vectorized.toarray()

        return {
            "text_features": vectorized
        }
    ...
    def process(self, query, INTENT_RANKING_LENGTH=5):
        ...
        if self.session is None:
            print("There is no trained tf.session: "
    ...
    def load(cls, model_dir=None, use_word_vectors=False):
        if model_dir:
            ...
            if not os.path.exists(os.path.join(model_dir, "checkpoint")):
                print("Failed to load nlu model. Maybe path {} "
                      "doesn't exist"
...
if __name__ == '__main__':
    ...
    train_text = DataHelper.generate_text(train_data)
    test_text = DataHelper.generate_text(test_data)

    intent_classifier = EmbeddingIntentClassifier()

    intent_classifier.train(train_text, train_label)

    intent_classifier.persist(model_dir='/Users/mac/PycharmProjects/cooking-kernel/classifier')

    # intent_classifier = intent_classifier.load(model_dir='/Users/mac/PycharmProjects/cooking-kernel/classifier')

    y_pred = []
    test_id = []
    for query, t_id in zip(test_text, test_data.id):
        result = intent_classifier.process(query)
        y_pred.append(result[0]['intent'])
        test_id.append(t_id)

        # Submission
    sub = pd.DataFrame({'id': test_id, 'cuisine': y_pred}, columns=['id', 'cuisine'])
    sub.to_csv('starspace_output2.csv', index=False)
