...
glob_files = './*_output.csv'
loc_outfile = './kaggle_vote.csv'
...
def kaggle_bag(glob_files, loc_outfile, method="average", weights="uniform"):
    ...
    for j, k in sorted(scores):
        ...
        id, ts = (k, Counter(scores[(j, k)]).most_common(1)[0][0])
        ids.append(id)
        target.append(ts)

    sub = pd.DataFrame({'id': ids, 'cuisine': target}, columns=['id', 'cuisine'])
    sub.to_csv('kaggle_vote.csv', index=False)
